...
private extension EmailTreatmentClassifier {
  ...
  private static func isEmailServiceProvider(_ domain: String) -> Bool {
    let domains = ["amazonses.com", "beehiiv.com", "cmail1.com", "constantcontact.com", "convertkit-mail.com", "hubspotemail.net", "klaviyomail.com", "mailchimpapp.net", "mailgun.org", "mandrillapp.com", "sendgrid.net", "substack.com"]
  ...
  private static func transactionalKind(
    ...
    if financeMarkers.contains(where: haystack.contains), hasFinanceSenderShape(provenance) {
      return .finance
    }
...
private extension EmailTreatmentClassifier {
  private static func hasAutomatedSenderShape(_ provenance: GmailArtifactProvenance?) -> Bool {
    ...
    let automatedLocalParts = ["account", "accounts", "attservices", "autopay", "automated", "billpay", "billing", "deposit", "deposits", "dispatch", "ebill", "fulfillment", "info", "mcinfo", "myuncchart", "notification", "notifications", "order", "orders", "reservation", "reservations", "shipping", "shippingnotification", "tracking", "delivery", "mailerdaemon"]
    return normalized.contains("noreply") || normalized.contains("donotreply")
      || automatedLocalParts.contains(normalized) || hasFinanceSenderShape(provenance)
      || hasShipmentSenderShape(provenance)
  }

  private static func hasFinanceSenderShape(_ provenance: GmailArtifactProvenance?) -> Bool {
    let financeDomains = ["americanexpress.com", "att.com", "bankofamerica.com", "bofa.com", "capitalone.com", "chase.com", "citi.com", "discover.com", "paypal.com", "wellsfargo.com"]
    let financeDomainTokens = ["bank", "banking", "bill", "billing", "ebill", "invoice", "payment"]
    let financeLocalParts = ["account", "accounts", "autopay", "billpay", "billing", "deposit", "deposits", "ebill", "payment", "payments", "statement", "statements"]
  ...
  private static func hasShipmentSenderShape(_ provenance: GmailArtifactProvenance?) -> Bool {
    ...
    let shipmentLocalParts = ["delivery", "dispatch", "fulfillment", "info", "mcinfo", "order", "orders", "shipping", "shippingnotification", "tracking"]
    let domains = senderDomains(provenance)
    let localPart = senderLocalPart(provenance)
    return domains.contains { domain in
  ...
  private static func typeHaystack(for piece: ContentPiece) -> String {
    let subjects = (try? JSONDecoder().decode([String].self, from: Data((piece.subjects ?? "").utf8)))?.joined(separator: " ") ?? ""
    return [piece.title, piece.summary ?? "", subjects].joined(separator: " ").lowercased()
  }

  private static let ephemeralMarkers = ["verification code", "sign-in code", "signin code", "security code", "one-time code", "one time code", "login code", "your code is", "otp"]

  private static let referenceMarkers = ["booking confirmation", "confirmation number", "delivery update", "delivery notice", "hotel confirmation", "invoice", "order confirmation", "payment received", "receipt", "reservation confirmation", "shipment", "shipping notice", "trade-in"]

  private static let referenceWithAutomatedSenderMarkers = ["appointment", "estimate", "visit"]

  private static let financeMarkers = ["account notice", "amount due", "autopay", "bill", "deposit", "e-bill", "ebill", "invoice", "mobile check deposit", "payment", "statement"]

  private static let shipmentMarkers = ["delivery", "on its way", "order confirmation", "order has been received", "order received", "out for delivery", "package", "shipped", "tracking", "your order"]
  ...
  private static func hasPromotionalSignal(_ piece: ContentPiece) -> Bool {
    let haystack = typeHaystack(for: piece)
    let markers = ["allocation", "case sale", "discount", "free shipping", "limited release", "new arrival", "offer", "pre-order", "sale", "save ", "shop now", "wine club"]
